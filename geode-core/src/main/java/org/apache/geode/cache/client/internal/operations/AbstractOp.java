...
public abstract class AbstractOp implements Op {
   ...
   *
   * @param connection the connection to use when sending
  ...
  protected void attemptSend(Connection connection) throws Exception {
    ...
    if (logger.isTraceEnabled(LogMarker.DISTRIBUTION_BRIDGE_SERVER_VERBOSE)) {
      logger.trace(LogMarker.DISTRIBUTION_BRIDGE_SERVER_VERBOSE, "Sending op={} using {}",
          getShortClassName(), connection);
    }
    getMessage().setComms(connection.getSocket(), connection.getInputStream(), connection.getOutputStream(),
  ...
  protected void processSecureBytes(Connection connection, Message message) throws Exception {
    if (connection.getServer().getRequiresCredentials()) {
      if (!message.isSecureMode()) {
        ...
        if (logger.isTraceEnabled(LogMarker.BRIDGE_SERVER_VERBOSE)) {
          logger.trace(LogMarker.BRIDGE_SERVER_VERBOSE,
              "Response message from {} for {} has no secure part.", connection, this);
        }
        return;
      }
      ...
      byte[] bytes = connection.decryptBytes(partBytes);
      DataInputStream dis = new DataInputStream(new ByteArrayInputStream(bytes));
      connection.setConnectionID(dis.readLong());
    }
  }
