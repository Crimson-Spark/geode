...
public class AuthenticateUserOp {
  ...
  static class AuthenticateUserOpImpl extends AbstractOp {
    ...
    public AuthenticateUserOpImpl(Connection connection, ExecutablePool executablePool) {
      ...
      DistributedMember server = new InternalDistributedMember(connection.getSocket().getInetAddress(),
      ...
      try {
        ...
        credentialBytes = connection.encryptBytes(heapdos.toByteArray());
      } catch (Exception e) {
    ...
    protected void sendMessage(Connection connection) throws Exception {
      ...
      try {
        secureBytes = connection.encryptBytes(hdos.toByteArray());
      } finally {
        hdos.close();
      }
    ...
    protected Object attemptReadResponse(Connection connection) throws Exception {
      ...
      if (msg != null) {
        msg.setComms(connection.getSocket(), connection.getInputStream(), connection.getOutputStream(),
            connection.getCommBuffer(), connection.getStats());
        if (msg instanceof ChunkedMessage) {
          try {
            return processResponse(connection, msg);
          } finally {
            msg.unsetComms();
            processSecureBytes(connection, msg);
          }
        } else {
          try {
            msg.receive();
    ...
    protected Object processResponse(Connection connection, Message msg) throws Exception {
      byte[] bytes = null;
      ...
      if (msgType == MessageType.RESPONSE) {
        ...
        if (bytes.length == 0) {
          ...
          byte[] decrypted = connection.decryptBytes(bytes);
